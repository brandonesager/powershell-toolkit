...
.EXAMPLE
    .\Invoke-WorkstationStabilityDiagnostic.ps1

.EXAMPLE
    .\Invoke-WorkstationStabilityDiagnostic.ps1 -Focus Shutdown -DaysBack 14

.EXAMPLE
    .\Invoke-WorkstationStabilityDiagnostic.ps1 -Focus Hardware -GenerateMonitoringScript -CreateEvidencePack

.NOTES
    Date: 2026-02-06
    Category: Diagnostics
.KEYWORDS
    diagnose, stability, freeze, shutdown, hardware
#>
...
function Add-RedFlag {
    param([string]$Message)
    $script:RedFlags += "[!] $Message"
}
...
function New-MonitoringScript {
    $scriptPath = "C:\Temp\Monitor-SystemHealth.ps1"
    $content = '$LogPath = "C:\Temp\SystemHealthLog.csv"; $Entry = [PSCustomObject]@{Timestamp = Get-Date}; $Entry | Export-Csv -Path $LogPath -Append -NoTypeInformation'
    ...
    foreach ($disk in $PhysicalDisks) {
        ...
        try {
            ...
            if ($ReliabilityCounter) {
                $DriveHealth += "  Temperature: $($ReliabilityCounter.Temperature) C"
...
# Save to file
$OutputPath = "C:\Temp\Workstation-Diagnostics-$(Get-Date -Format 'yyyyMMdd-HHmmss').txt"
