<#
.SYNOPSIS
    Contoso Active Directory User Onboarding Script

.DESCRIPTION
    Automates the creation of new Active Directory user accounts for Contoso.
    This script creates user accounts, assigns mandatory groups, mirrors permissions from existing users,
    and triggers Azure AD Connect synchronization.
    
    .VERSION
    1.0
    
    .REQUIREMENTS
    - PowerShell 5.1 (Windows PowerShell - required for system context)
    - Active Directory PowerShell module
    - Domain controller or server with AD management tools
    - Azure AD Connect (for sync functionality)
...
.PARAMETER Department
    Department name (e.g., "Operations", "IT", "Facilities")
...
.PARAMETER OULocation
    'MainOffice' or 'RemoteSites' - determines OU placement
...
.NOTES
    ...
    EXECUTION CONTEXT: This script must run in PowerShell 5.1 as SYSTEM account
    ENVIRONMENT: Domain controller or AD management server
    LOGGING: Creates detailed logs at C:\Temp\OnboardAD_timestamp.log

.KEYWORDS
    AD, provision, user, onboarding
#>
...
param(
    ...
    [string]$Department = "Operations",                        # CHANGE THIS: User's department
    
    [Parameter(HelpMessage = "Job title")]
    [string]$JobTitle = "Assistant Manager",                    # CHANGE THIS: User's job title
    ...
    [string]$StreetAddress = "STREET_ADDRESS_PLACEHOLDER",     # CHANGE THIS: Full street address
    ...
    [string]$State = "CA",                                     # Usually CA for Contoso properties
    
    [Parameter(HelpMessage = "ZIP code")]
    [string]$ZipCode = "ZIP_CODE_PLACEHOLDER",                 # CHANGE THIS: ZIP code
    
    [Parameter(HelpMessage = "Country code")]
    [string]$Country = "US",                                   # Standard: United States
    
    [Parameter(HelpMessage = "Office name")]
    [string]$Office = "OFFICE_NAME_PLACEHOLDER",               # CHANGE THIS: Office name
    ...
    [Parameter()]
    [ValidateSet('MainOffice', 'RemoteSites')]
    ...
    [string]$UpnSuffix = "@contoso.com"                    # Standard: organization domain suffix
)
...
try {
    ...
    # Follows Contoso naming convention: first initial + last name (lowercase)
    # Must check for conflicts before proceeding with account creation
    #--------------------------------------------------------------------------
    
    # Generate username following Contoso standard format
    ...
    if ($OULocation -eq "MainOffice") {
        $MandatoryGroups += "Contoso-Main Printers"
        Write-Log "Adding Contoso-Main Printers group (Main Office location)"
    }
    
    # Role-specific groups
    switch ($UserRole) {
        'MaintenanceTechnician' {
            $MandatoryGroups += @("All Maintenance", "AllMaintenanceENG", "AllStaff", "AllStaffENG")
