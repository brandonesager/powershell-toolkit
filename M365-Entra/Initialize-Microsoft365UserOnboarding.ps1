...
.DESCRIPTION
    ...
    - Role-based shared mailbox assignment (by job title/department)
...
param(
    ...
    [Parameter(HelpMessage = "Department (e.g., Operations, IT, Facilities)")]
    [string]$Department = "DEPARTMENT_PLACEHOLDER",                # CHANGE THIS: User's department
    
    [Parameter(HelpMessage = "Job title (e.g., Assistant Manager, Field Technician)")]
    ...
    [Parameter(HelpMessage = "Shared mailbox names to assign permissions (e.g., 'Building-A','Operations')")]
...
try {
    ...
    $SharedMailboxRoles = @(
        ...
        'Field Technician', 'Facilities', 'Maintenance'
    )
    ...
    if ($RequiresSharedMailboxes -and -not [string]::IsNullOrWhiteSpace($MgUser.OfficeLocation)) {
        ...
        try {
            ...
            # Remove common name suffixes and create variations
