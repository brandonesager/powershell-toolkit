<#
.SYNOPSIS
    Identifies inactive users in hybrid AD environments.

.DESCRIPTION
    Cross-references on-premises Active Directory with Exchange Online Unified
    Audit Log to identify truly inactive users. Checks enabled users with expiring
    passwords who haven't logged in for 90+ days and have no recent EXO activity.
    
    Helps identify accounts for cleanup or offboarding in hybrid environments.
...
.EXAMPLES
    Example 1: Run with defaults
        Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass -Force; & 'c:\temp\Get-HybridAdInactiveUsers.ps1'
